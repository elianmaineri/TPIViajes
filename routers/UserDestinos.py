...
@destinos_router.get('/ALL-DESTINOS', tags=['Destinos'], response_model=List[Destinos], status_code=200)
...
@destinos_router.get('/NOMBRE-DESTINOS', tags=['Destinos'], response_model=Destinos, status_code=200)
...
@destinos_router.get('/PAIS-DESTINOS', tags=['Destinos'], response_model=Destinos, status_code=200)
...
def update_destinos(id: int, destino: Destinos):
    db = Session()
    mod_destino = DestinosServices(db).update_destinos(id, destino)
    if not mod_destino:
